...
def train_negrad(epoch, train_loader, delete_loader, model, criterion, optimizer, alpha, quiet=False):
    """Negative Gradient Training with MedMNIST compatibility"""
    model.train()
    ...
    for idx, ((input, target), (del_input, del_target)) in enumerate(zip(train_loader, cycle(delete_loader))):
        ...
        # Handle multi-dimensional targets for MedMNIST
        if target.dim() > 1:
            target = target.squeeze()  # Squeeze target to correct shape
        if del_target.dim() > 1:
            del_target = del_target.squeeze()
...
def train_vanilla(epoch, train_loader, model, criterion, optimizer, quiet=False):
    """Vanilla training with MedMNIST compatibility"""
    model.train()
    ...
    for idx, (input, target) in enumerate(train_loader):
        ...
        # Handle multi-dimensional targets for MedMNIST
        if target.dim() > 1:
            target = target.squeeze()  # Squeeze to handle cases where target is multi-dimensional
...
def train_distill(epoch, train_loader, module_list, swa_model, criterion_list, optimizer, split, quiet=False):
    """One epoch distillation with MedMNIST compatibility"""
    gamma = 1
    alpha = 0.5
    beta = 0
    smoothing = 0.5
    ...
    for idx, data in enumerate(train_loader):
        # Handle different distillation methods
        if distill in ['crd']:
        ...
        # Handle multi-dimensional targets (e.g., MedMNIST)
        if target.dim() > 1:
            target = target.squeeze()

        # ===================forward=====================
        logit_s = model_s(input)  # Student output
        with torch.no_grad():
            logit_t = model_t(input)  # Teacher output (detached)

        # Compute classification loss and distillation loss
...
def validate(val_loader, model, criterion, quiet=False):
    """Validation"""
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
